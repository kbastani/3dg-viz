...
function initGraphSettings() {
  myWorker.settings = {
    ...
    nodeSize: new Float32Array(50000 * 1),
...
function resetGraphSettings() {
  ...
  myWorker.settings.nodeSize.clear();
...
function init() {
  ...
  uniforms = {
    ...
    texture:   { type: "t", value: THREE.ImageUtils.loadTexture( "/assets/textures/ball.png" ) }
  ...
  myWorker.settings.geometry.addAttribute('customColor', new THREE.BufferAttribute(myWorker.settings.colors, 3));
  myWorker.settings.geometry.addAttribute('size', new THREE.BufferAttribute(myWorker.settings.nodeSize, 1));
  ...
  myWorker.settings.particleSystem = new THREE.PointCloud(myWorker.settings.geometry, shaderMaterial);
...
function addParticle(e) {
  ...
  myWorker.settings.nodeSize[myWorker.settings.particles / 3] = Math.min(Math.max(e.data[3] * 1000, 600), 3000);
...
function render() {
  if(onRender) {
    ...
    if (rotateGraph) {
      ...
      myWorker.settings.particleSystem.geometry.attributes.customColor.needsUpdate = true;
      myWorker.settings.particleSystem.geometry.attributes.size.needsUpdate = true;
